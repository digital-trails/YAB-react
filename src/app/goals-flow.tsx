      ...
      {page === 'home' ? <Home onPick={(nextPage) => { setPage(nextPage); setStep(0); }} /> : (
        <>
          ...
          <View style={[styles.footer, { paddingBottom: insets.bottom + 16 }]}>
            <Btn label={step >= 3 ? 'Finish' : 'Continue'} disabled={step < 3 && !selected && !(page === 'values' && step === 1)} onPress={step >= 3 ? finish : next} style={step < 3 && !selected && !(page === 'values' && step === 1) ? styles.disabled : undefined} />
          </View>
        </>
      )}
    </View>
...
function Values({ step, selected, onSelect, text, setText }: { step: number; selected: string | null; onSelect: (item: string) => void; text: string; setText: (value: string) => void }) {
  ...
  if (step === 0) return <><Text style={styles.kicker}>VALUE</Text><Text style={styles.body}>A value is something you care about and want to guide your choices, such as health, family, friendship, learning, or being kind to yourself.</Text><Heading style={styles.question}>What value do you care about?</Heading><Choices items={values} selected={selected} onSelect={onSelect} /><Btn label="Need an idea?" variant="ghost" onPress={() => setShowIdeas((shown) => !shown)} />{showIdeas ? <View style={styles.ideaList}>{['Being there for people I care about', 'Learning something new', 'Taking care of my wellbeing'].map((idea) => <Pressable key={idea} onPress={() => onSelect(idea)} style={styles.idea}><Text style={styles.body}>{idea}</Text></Pressable>)}</View> : null}</>;
